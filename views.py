...
from AdminUI.models import StudentDB,DepartmentDB,CourseDB,EnrollmentDB
...
def stud_profile(request):
    ...
    data4 = EnrollmentDB.objects.all()
    return render(request,"student_profile.html",{'data1':data1,'data2':data2,'data3':data3,'data4':data4})
...
def stud_edit(request):
    ...
    data3 = CourseDB.objects.all()
    data4 = EnrollmentDB.objects.all()
    return render(request,"student_edit.html",{'data1':data1,'data2':data2,'data3':data3,'data4':data4})
    ...
    if request.method=="POST":
        ...
        if StudentDB.object.filter(StudentId=stud_id).exits():
            StudentDB.objects.filter(StudentId=stud_id).update(FirstName=stud_fname,LastName=stud_lname,
                                                               ...
                                                               GuardianContact=stud_gcontact,DeptName=stud_dept,
